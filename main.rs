fn maximum_odd_binary(s: &[u8]) -> Vec<u8> {
    ...
    // u8 accumulator per 255-byte chunk lets LLVM use full 32-byte AVX2 lanes
    // without widening each element to usize to prevent overflow.
    let ones: usize = s
        .chunks(255)
        .map(|chunk| chunk.iter().map(|&b| b & 1).sum::<u8>() as usize)
        .sum();
